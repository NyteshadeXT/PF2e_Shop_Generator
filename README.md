...
Generation validation, deterministic selection orchestration, and snapshot assembly live in `services/generation.py`. Spell-scroll and wand enrichment lives in `services/spell_items.py`. Shared equipment-context and prerequisite matching for weapon, armor, and shield runes lives in `services/rune_prerequisites.py`; rune classification, level caps, candidate construction, and weighted selection live in `services/rune_selection.py`. Applying those runes—including composed price, rarity, level, category, and display names—lives in `services/runed_equipment.py`. The remaining catalog-selection rules stay in `services/logic.py`. The Magic Item Builder API is registered from `services/magic_builder.py`. Player-facing displays, stored GM results, publishing, and campaign archive routes live in `services/player_view_routes.py`, while durable storage remains in `services/player_views.py`. Authentication, shared sessions, CSRF enforcement, and browser security headers are configured by `services/web_security.py`. Canonical inventory section names and the transformations shared by generation, curation, and result templates live in `services/inventory_sections.py`. Keeping these workflows outside `app.py` leaves the application module focused on application setup, generation requests, error presentation, health checks, and the smaller standalone tools.
